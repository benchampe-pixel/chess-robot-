...
from config import SERIAL_PORT, BAUD_RATE, PIECE_HEIGHTS
...
from robot import send_command, close_serial, go_to_point, get_current_point
...
def main():
    ...
    while True:
      ...
      # window for waitKey to work
      cv2.imshow("Calibration - Press 'r' to record point", frame)

      if cv2.waitKey(1) & 0xFF == ord('r'):
        # append point to the list
        point = get_current_point()
        board_calibration_locations.append(point)
        print("Calibration point added:", point)

        if (len(board_calibration_locations) == 4):
            print("Calibration complete.")
            # print out board
            for i, p in enumerate(board_calibration_locations):
                print(f"Corner {i+1}: {p}")
            break
        
      time.sleep(0.02)

    square_centers, square_locations = get_square_centers(board_corners, board_calibration_locations)
